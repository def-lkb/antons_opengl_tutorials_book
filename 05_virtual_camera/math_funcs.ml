...
module M4 = struct
  ...
  let translate m {V3. x; y; z} =
    let m_t = {identity with m = x; n = y; o = z} in
    m_t *% m

  let rotate_x_deg m deg =
    let rad = deg *. one_deg_in_rad in
    let c = cos rad and s = sin rad in
    let m_r = {identity with f = c; j = -.s; g = s; k = c} in
    m_r *% m
  ...
  let rotate_z_deg m deg =
    let rad = deg *. one_deg_in_rad in
    let c = cos rad and s = sin rad in
    let m_r = {identity with a = c; e = -.s; b = s; f = c} in
    m_r *% m

  let scale m {V3. x; y; z} =
    let m_s = {identity with a = x; f = y; k = z} in
    m_s *% m
  ...
  let look_at cam_pos targ_pos up =
    ...
    let open V3 in
    (* inverse translation *)
    let p = translate identity (cam_pos *% -1.) in
    (* distance vector *)
