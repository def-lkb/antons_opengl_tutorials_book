...
let capture x y w h =
  let data = Bigarray.(Array1.create int8_unsigned c_layout) (w * h * 3) in
  Gl.read_pixels x y w h Gl.rgb Gl.unsigned_byte (`Data data);
  let img = {Stb_image. channels = 3; width = w; height = h; data} in
  flip_image img;
  Stb_image_write.png (Printf.sprintf "screenshot_%d.png"
