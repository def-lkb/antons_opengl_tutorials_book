...
let image_size = w * h * 3
let image_count = fps * duration
let buffer_size = image_size * image_count
let buffer = Bigarray.(Array1.create int8_unsigned c_layout) buffer_size
let image n =
  if n < image_count
  then Some (Bigarray.Array1.sub buffer (image_size * n) image_size)
  else None
...
let capture  =
  ...
  | None -> ()
  | Some data ->
    Gl.read_pixels 0 0 800 600 Gl.rgb Gl.unsigned_byte (`Data data);
    incr n
...
let dump_video () =
  for i = 0 to image_count - 1 do
    ...
    | None -> ()
    | Some data ->
      let img = {Stb_image. channels = 3; width = w; height = h; data} in
      flip_image img;
      Stb_image_write.png (Printf.sprintf "video_%d.png" i)
        ~w:800 ~h:600 ~c:3 data
  done
...
let main () =
  ...
  (* load texture *)
  let _tex = load_texture "skulluvmap.png" in
  ...
  let want_video = ref false in
  let video_timer = ref 0.0 in
  let video_dump_timer = ref 0.0 in
  let frame_time = 1.0 /. float_of_int fps in

  (* The main event loop *)
  let rec loop () =
    let is_done =
      !video_timer > float_of_int duration ||
    ...
    if is_done then ()
      ...
      begin
        ...
        if !want_video then
          begin
            video_timer := !video_timer +. elapsed_seconds;
            video_dump_timer := !video_dump_timer +. elapsed_seconds;
          end;
        ...
        if check_key Sdl.Scancode.f11 then
          want_video := true;
        ...
        if !want_video then
            while !video_dump_timer > frame_time do
              capture ();
              video_dump_timer := !video_dump_timer -. frame_time
            done;
